...
function Kpi({ label, value, unit, sub, labelColor = 'text-fuel-primary', children }: KpiProps) {
  return (
    <div className="min-w-0 flex flex-col justify-center rounded-lg border border-industrial-700 bg-industrial-800 px-2 py-1.5 xl:px-3 xl:py-2">
      <div className={`truncate text-[9px] xl:text-[10px] 2xl:text-[11px] font-semibold tracking-wider ${labelColor}`}>{label}</div>
      <div className="flex items-baseline gap-1">
        <span className="font-mono font-bold leading-none text-white text-lg xl:text-2xl 2xl:text-[26px]">{value}</span>
        {unit && <span className="text-[9px] xl:text-[11px] text-gray-400">{unit}</span>}
      </div>
      {sub && <div className="mt-0.5 text-[9px] xl:text-[10px] text-gray-500">{sub}</div>}
      {children}
    </div>
  )
}
...
export default function FuelKPIs({
  ...
  return (
    <div className="grid flex-none grid-cols-3 gap-2 md:grid-cols-6 xl:gap-3">
      ...
      <Kpi label="CONSUMO EN RALENTÍ HOY" value={idleToday.toFixed(1)} unit="gal" sub={`(${idlePct.toFixed(1)}%)`} />
