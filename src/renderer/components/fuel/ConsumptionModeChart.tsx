    ...
    <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-industrial-700 bg-industrial-800 md:col-span-3">
      ...
      <div className="flex min-h-0 flex-1 flex-col p-2 xl:p-3">
        <div className="flex min-h-0 flex-1 items-center gap-3">
          ...
          <ul className="flex min-w-0 flex-1 flex-col gap-1 md:gap-0.5 lg:gap-1 xl:gap-2">
            {data.map(s => (
              <li key={s.name} className="flex gap-1">
                <span className="mt-0.5 h-2.5 w-2.5 flex-none rounded-sm" style={{ backgroundColor: s.color }} />
