...
export default function BottomNavigation({ className = '' }: BottomNavigationProps) {
  ...
  const isBlockedView = (id: Page) => {
    if (isExcavator) return false
    return id=== 'fuel' || id === 'operation' || id === 'productivity' || id === 'alerts'
  }
    ...
    >
      <div className="grid grid-cols-5 w-full">
        {navItems.map((item, idx) => {
          ...
          const isDisabled = isBlockedView(item.id)
          return (
            <div key={item.id} className="relative flex">
              ...
              <button
                ...
                title={isDisabled ? 'No disponible para esta maquinaria' : undefined}
              >
                {isActive && (
                ...
                <span className={`text-[10px] md:text-[11px] lg:text-xs font-bold tracking-wide uppercase leading-none truncate max-w-[7.5rem] ${
                  isDisabled ? 'text-gray-500' : isActive ? 'text-electric-300' : 'text-gray-300 group-hover:text-white'
                }`}>
                  {item.label}
                </span>
                {isActive && !isDisabled && (
